...
int main()
{
  ...
  while (true)
  {
    ...
    if (screen_size.first % 2 == 0)
      screen_size.first--;
    if (screen_size.second % 2 == 0)
      screen_size.second--;
  ...
  {
    if (s.size() > biggest_str_len)
      biggest_str_len = s.size();
  }
    ...
    if (input)
    {
      ...
      for (string s : menu_text)
      {
        ...
        for (char c : s)
        {
          ...
          if (menu_option_number == selected)
            t.setChar(center.first + row_offset, cursor_column, menu_option_number + 48, CURSOR_BOLD, CURSOR_ITALIC, CURSOR_UNDERLINE, CURSOR_BLINK, CURSOR_FG_COLOR, CURSOR_BG_COLOR);
          else
            t.setChar(center.first + row_offset, cursor_column, menu_option_number + 48, MENU_BOLD, MENU_ITALIC, MENU_UNDERLINE, MENU_BLINK, MENU_FG_COLOR, MENU_BG_COLOR);
